...
class UserController {
    ...
    public async saveAuthorHandler(req: Request, res: Response, next: NextFunction): Promise<Response> {
        let { userId } = res.locals
        let { githubLink = "", stackoverflowLink = "", twitterLink = "", name = "", image = "", image_webp = "", bio = "" }: AuthorInterface = req.body
        try {
            let author = await userService.saveAuthor(userId, { githubLink, stackoverflowLink, twitterLink, name, image, image_webp, bio })
            return res.status(200).json(author)
        } catch (err) {
